...
class Organism:
    ...
    def find_correspondences(self, f_name):
            ...
            while line:
                if line.startswith(">"):
                    ...
                    while not newline.startswith(">") and newline:
                        ...
                        if current_name.startswith("orf"):
                            orfs.append(current_name)
                        else:
                            correspondences.append(current_name)
                        newline = f.readline()
                    else:
                        if orfs:
                            if correspondences:
                                for i in orfs:
                                    r[i] = np.unique(correspondences)
                        line = newline

        return r

    # cette fonction est nouvelle
    def write_cluster_fasta_files(self):
         for index, item in enumerate(self.clusters):
            dict = self.find_gene_sequence(index + 1)
            with open(self.path +"/" + "cluster_" + str(index + 1) + ".fasta", "w") as f:
    ...
    def find_ids(self):
        prodig_r = self.path + "/" + "orfs_protein.fasta"
        for index, item in enumerate(self.clusters):
            doc = self.path + "/" + "cluster_" + str(index + 1) + ".fasta"
            prefix = self.path + "/" + "cluster_" + str(index + 1)
            cmd = "cdhit-2d -i2 " + prodig_r + " -i " + doc + " -o " + prefix + "  -c 1 -n 5  -M 1600 -d 0"
            print cmd
            os.system(cmd)
            cdhit_r = prefix + ".clstr"
            r = self.find_correspondences(cdhit_r)
            print r
            for values in r.values():
                for gn in values:
                    item.contig_name.append(gn)
    ...
    # Only cd-hit clusters with a gene detected by prism will be annotate ....
...
# Useful to parse more than one genome prism results...
