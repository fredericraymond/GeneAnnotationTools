                        ...
                        for gr in orf:
                            if gene in gr:
                                for corr in gr[1:]:
                                    cpt.append(corr)
                    cpt = np.unique(cpt)
                    pseudo_clstr[contig][pos] = (pseudo_clstr[contig][pos], len(cpt))
...
def get_stats(correspondences, group_by):
    ...
    for p in group_by.keys():
        ...
        if percent > 0:
            results[p] = str(stats[p]) + "/" + str(len(group_by[p]))  #int(round(percent, 0))
...
def display_cluster_density(wg, f_name, group_by):
    ...
    with open(f_name, "w") as f:
        f.write("\t".join(["Contigs ", "genes", "Pathways", "Pathways_length ", "cluster density"]))
        f.write("\n")
        for c, values in res.items():
            for p, liste in values.items():
                for clstr in liste:
                    ...
                    mem1 = ",".join(series)
                    mem2 = clstr[-1]
                    line = "\t".join([str(c), str(mem1), str(p), str(len(group_by[p])), str(mem2)])
                    f.write(line)
                    f.write("\n")
